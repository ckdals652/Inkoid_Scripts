...
public class HockHead : MonoBehaviourPun
{
    ...
    [Header("Hook Shoot Effect")]
    [SerializeField] private PrefabIndex_Effect hookShootEffect = PrefabIndex_Effect.ImpactFX_RPG;
    [SerializeField] private float hookShootEffectLifetime = 1.5f;
    [SerializeField] private float hookShootEffectScale = 1f;
    [SerializeField] private float hookShootEffectForwardOffset = 0.6f;
    ...
    private bool hasAuthority;                 // [변경]
    private bool isLocalHuman;                 // [변경]

    // 모바일 최적화: Texture2D 캐싱 (매번 생성하지 않고 재사용)
    ...
    private void Awake()
    {
        hasAuthority = photonView.IsMine;          // [변경]
        isLocalHuman = hasAuthority && playerHandler != null && !playerHandler.IsBot; // [변경]
    ...
    private void Start()
    {
        ...
        hasAuthority = photonView.IsMine;
        isLocalHuman = hasAuthority && playerHandler != null && !playerHandler.IsBot;
    }
    ...
    private void OnEnable()
    {
        ...
        // 혹시 없으면 다시 찾아놓기(리스폰/파괴 후)
        if (!firePosition) firePosition = player ? player.Find(hockFirePositionText) : null;
        if (!ropeLineRenderer && player)
            ropeLineRenderer = player.Find(ropeRendererText)?.GetComponent<LineRenderer>();
    }
    ...
    private void Update()
    {
        ...
        if (!player || !playerHandler) return;

        // 모바일 최적화: Idle 상태일 때는 라인 업데이트 불필요
        if (currentState != HockState.Idle)
        {
            UpdateRopeLine();
        }
    }
    ...
    private void FixedUpdate()
    {
        // 널/소유권 가드
        if (!this || !enabled) return;
        if (!playerHandler || !playerRigidbody || !hockRigidbody) return;
        ...
        switch (currentState)
        {
            ...
            case HockState.Shot:
                // 모바일 최적화: sqrMagnitude 사용 (제곱근 연산 제거)
                if ((player.position - transform.position).sqrMagnitude >= hookPullMaxRange * hookPullMaxRange)
                    SetState(HockState.Returning);
                break;

            case HockState.Grabbed:
                // 모바일 최적화: sqrMagnitude 사용 (제곱근 연산 제거)
                if ((player.position - transform.position).sqrMagnitude >= hookPullLimit * hookPullLimit)
                {
    ...
    private void TryShoot()
    {
        ...
        SpawnHookShootEffect();
    }

    private void MovePlayerToHook()
    {
        // 드릴이 사용/해제 시 이속 제한을 변화시키기에 꼬임 >> 훅 사용 중 계속 값을 써주게 하여 문제 해결
    ...
    {
        // ★ 라인/파이어포인트가 파괴되었거나 아직 준비 안됐으면 스킵
        if (!ropeLineRenderer || !firePosition || !gameObject || !gameObject.activeInHierarchy) return;

        // 훅을 사용하기 시작했을 때 라인랜더러 enable
        if(!ropeLineRenderer.enabled) ropeLineRenderer.enabled = true;

        if (ropeLineRenderer.positionCount < 2) ropeLineRenderer.positionCount = 2;
    ...
    private void SetState(HockState newState)
    {
        ...
        switch (newState)
        {
            ...
            case HockState.Returning:
                ...
                InkEvents.ResetInkRecovery(photonView.ViewID);
                if (isLocalHuman) SoundManager.Instance.StopLoopSfx();
                break;
        }
    }
    private void SpawnHookShootEffect()
    {
        if (!RPCManager.Instance || !firePosition) return;
        if (hookShootEffectLifetime <= 0f) return;

        Vector3 forward = shootDir.sqrMagnitude > 0f ? shootDir.normalized : firePosition.forward;
        Vector3 spawnPos = firePosition.position + forward * hookShootEffectForwardOffset;
        Quaternion spawnRot = Quaternion.LookRotation(forward);

        RPCManager.Instance.photonView_RPCManager.RPC(
            nameof(RPCManager.Instance.SpawnEffect_VFXOnly),
            RpcTarget.AllViaServer,
            myTeam,
            hookShootEffect,
            hookShootEffectLifetime,
            spawnPos,
            spawnRot,
            hookShootEffectScale);
    }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (((1 << collision.gameObject.layer) & grappleableLayers) != 0)
        {
            ...
            if (paintableObject != null)
            {
                for (int i = 0; i < rayCount; i++)
                {
                    ...
                    if (Physics.Raycast(ray, out hit, 1f, (1 << (int)EnumLayer.LayerType.Map_Printable)))
                    {
                        ...
                        // 모바일 최적화: Texture2D 캐싱 및 재사용
                        if (_cachedTexture == null || _cachedTextureWidth != rt.width || _cachedTextureHeight != rt.height)
                        {
                            if (_cachedTexture != null)
                                Destroy(_cachedTexture);
                            _cachedTexture = new Texture2D(rt.width, rt.height, TextureFormat.RGBA32, false);
                            _cachedTextureWidth = rt.width;
                            _cachedTextureHeight = rt.height;
                        }

                        RenderTexture prev = RenderTexture.active;
                        RenderTexture.active = rt;
                        _cachedTexture.ReadPixels(new Rect(0, 0, rt.width, rt.height), 0, 0);
                        ...
                        Color teamPosession = _cachedTexture.GetPixel(x, y);
                        ...
                        if (painted == 1 && (int)myTeam == teamID)
                        {
                            ...
                            // //Debug.Log("잉크 부스트");
                            if (photonView != null)
                                InkEvents.BoostInkRecovery(photonView.ViewID);
                            break;
                        }
                    }
                    else
            ...
            SoundManager.Instance.PlaySfx3D(ClipIndex_SFX.mwfx_HookHit, transform.position);
        }
        else
        {
            //붙을수 없는 놈일 때 바로 돌아가게
            SetState(HockState.Returning);
            // 튕기는 사운드 재생은 이 자리에
            SoundManager.Instance.PlaySfx3D(ClipIndex_SFX.mwfx_HookMiss, transform.position);
        }
    }
}